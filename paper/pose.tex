...
\beta_j = \frac{(-1)^j}{j}\exp\left(-\frac{j}{{\sf d}} \right)\\
&\text{and}~x_j \stackrel{ind}{\sim} \mr{Bernoulli}\left( 1/(1+e^{-z_j})\right) \text{for}~~j=1\dots p,~~\text{given}~~\bm{z} \sim \mr{N}\left(\bm{0},\bs{\Sigma}\right).\notag 
\end{align}
...
$\mr{sd}(\bs{\eta})/\sigma$ of $1/2$, $1$, and $2$. Finally, the coefficient decay  ${\sf d}$ controls how quickly $\beta_j$ goes to zero; we consider
