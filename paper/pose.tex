...
This appears under a variety of parameterizations and names in the
literature; see
...
\citet{cevher_learning_2009}, \citet{taddy_multinomial_2013} and \citet{armagan_generalized_2013}. 
 The
penalty is nondifferentiable at zero and is concave away from zero with curvature $c''(b) = -\gamma/(1+\gamma|b|)^2$ and gradient $c'(b) = 1/(1+\gamma|b|)$ (note that $\lim_{b\to 0} c'(b) = 1$ as required).  The penalty in (\ref{logpen})  spans the
range from $\ell_1$ to $\ell_0$ 
costs: L'Hopital's rule implies $\lim_{\gamma\to 0} c(b) = |b|$, while $\lim_{\gamma\to \infty} c'(b) = 0$ if $b\neq 0$ for a flat $\ell_0$ penalization (this latter case is complicated, because $\lim_{\gamma\to \infty} c(b) = 0$;  we also need $\lambda$ to grow with $\gamma/\log\gamma$ so that the cost doesn't just go to zero).
