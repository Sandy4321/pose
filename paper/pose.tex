...
The statistics literature of the past 15 years has established many favorable
 ...
 standard information criteria can be applied in penalty selection. We also provide novel results on the distance between weighted-$L_1$ and $L_0$ penalized predictors; this allows us to build intuition about POSE and other diminishing-bias regularization schemes.  The
 methods and results are illustrated in extensive simulations and in application of
...
\end{align}
\vskip -.3cm
\end{algorithm}\vskip .25cm}

In this, we are assuming a penalty specification such that $\lim_{b\to 0} c'(|b|) = 1$ and that the cost function is differentiable for $b\neq 0$.  
...
$j^{th}$ coefficient gradient evaluated at $\hat\beta_j=0$.\footnote{Note that
...
computation and has the advantage of maintaining $df =
\hat p^t$ for $\gamma = 0$.
...
nears $n$, indicating that the heuristic in (\ref{edf}) might underestimate $df^t$ for clearly over-fit models.
...
\vskip -.25cm
\caption{\label{tab:simsparse} Average out-of-sample $R^2$, reported as  \% worse than the $C_p$ oracle (on far right), across 1000 different samples from (\ref{simdgp}).   Each row of this table corresponds to a different data generating process (see supplement for more).  Lasso (GL $\gamma=0$), GL, and AL routines were executed in {\tt gamlr}.  MCP denotes results from the {\tt sparsenet} MCP solver with 5-fold CV selection. The best results are bolded.}
\end{table}
...
The OOS $R^2$ results are reported in terms of percentage worse than the oracle $C_p$ rule from (\ref{l0oracle}).  As a comparator of similarly low computational cost (again, little more expensive than a standard lasso) we also consider results for the {\it marginal} adaptive lasso with $L_1$ penalty weights\footnote{The AL weights are then scaled so that $\mathrm{min}(\omega_j)=1$.  This implies that each AL $\lambda$ path is the same as for any GL run on the same data (since this is the weight on the first coefficient to enter the regression, which dictates $\lambda^1$).} $\omega_j \propto |\mr{cor}(\bm{x}_j,\bm{y})|^{-1}$.  AICc selection for marginal AL just sets $df$ to the number of nonzero estimated parameters.   As a sort of `expensive' gold standard, we also compare to an exact solver for concave MCP penalized regression. As described in Section \ref{sec:weighted}, {\tt sparsenet} applies 5-fold CV to optimize out-of-sample error over a
dense grid of potential penalty sizes ($\lambda$) and concavities (analogous
to our $\gamma$) for the MCP penalty.\footnote{
...
with combined compute time still a fraction of a second,  yields a near-best predictor in all scenarios.  Moreover, we note that each GL run is completely independent and can be executed in parallel.

\vskip .1cm \noindent $\bullet$~~  For GL1, AICc gives roughly similar performance to 5-fold CV.  For the lasso (GL0), CV outperforms AICc in a dense (big {\sf d}) and high signal model, while AICc outperforms CV in low signal settings.  The opposite holds for GL10.   In any case, the supplement shows that AICc is a massive improvement over either BIC or the uncorrected AIC;  the
...
should get closer to that of an $L_0$ oracle if $\|\bs{\omega}_S\|$ -- the
norm on the weights in the support of the $L_0$ rule -- can be made small
without $\omega^{\mr{min}}_{S^c}$ -- the smallest weight on the complement of
this support -- shrinking too much.\footnote{Marginal AL provides an interesting side-case; it maintains higher
$\omega^{\mr{min}}_{S^c}$ at the expense of a {\it much} higher
$\|\bs{\omega}_S\|$. }   Comparing  lasso and GL1 lines, we see
