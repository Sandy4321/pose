...
The statistics literature of the past 15 years has established many favorable
 ...
 standard information criteria can be applied in penalty selection. Our contribution also includes novel results on the distance between weighted-$L_1$ and infeasible oracle $L_0$ penalized predictors; this allows us to build intuition about our POSE algorithms and other fast-to-compute diminishing-bias regularization schemes (such as the adaptive lasso).  The
...
\vskip .1cm \noindent $\bullet$~~   A single {\tt gamlr} run requires a fraction of a second; {\tt sparsenet} with 5-fold CV requires 15-20 seconds per run.  
  Investigation of AICc values finds that minimization of this
criteria {\it across} GL0, GL1, and GL10 would have consistently chosen the rule that
does best in Table \ref{tab:simsparse}.  Thus AICc selection on just three GL paths,
with combined compute time still a fraction of a second,  yields a near
best predictor in all scenarios.

\vskip .1cm \noindent $\bullet$~~  AICc and 5-fold CV give roughly the same performance for the GL0 (lasso) and GL1; AICc is often much better than CV for the less stable GL10 paths.  The supplement shows that AICc is a massive improvement over either BIC or the uncorrected AIC;  the
latter leads to negative $R^2$ in many configurations. 

\vskip .1cm \noindent $\bullet$~~  Selected GL2 fits had often half the number of nonzero
...
\vskip .1cm \noindent $\bullet$~~ As $\gamma$ increases, the CV error increases more quickly after
