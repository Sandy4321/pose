...
2l(\bs{\hat\beta}) + k(df)
\end{equation}
where $k(df)$ is the cost function on the degrees-of-freedom associated with
estimates $\bs{\hat\beta}$.  For example, the AIC of
\cite{akaike_information_1973} uses $k(df) = 2df$ while the BIC of \cite{schwarz_estimating_1978}
uses $k(df) = \log(n)df$.

As detailed in \cite{flynn_efficiency_2013}, the corrected AICc with $k(df) =
2df(n/n-df-1)$ does a much better job than the AIC or BIC in choosing the
