...
\noindent \vskip .2cm \noindent $\bullet$~~ Because we
have no notion of a `true' model, this result has nothing to say about the
estimation error on individual parameters.  We again refer the reader to
\cite{buhlmann_statistics_2011}, and note that many of their bounds on
estimation error for weighted-$\ell_1$ penalization rely upon weights derived
from model fits at smaller $\lambda$ than is used in the final estimation (i.e., the opposite of GL and POSE, where weights are derived from fits at larger
...
sample.  Our experiments consist of all possible combinations of the following configuration options:
\begin{itemize}
...
For comparison, we include performance results for MLE fit on `oracle' restricted support.  For the strictly sparse model, with $J=n/10$, our oracle uses the true nonzero support.  For the strictly dense model, our oracle comparator is the $C_p$ optimal $\ell_0$ penalized solution
...
all of our simulation models.  We include \texttt{gamlr} runs of GL with $\gamma$
of 0 (lasso), 1, and 10. For penalty selection, we use AICc with the GL $df$
from Section \ref{sec:select} as well as 5-fold CV (with $\lambda$ selected to
{\it minimize} CV error estimates). We also consider `GL select', a routine which chooses $\gamma \in \{0,1,10\}$ to minimize these AICc or CV values.
...
$\mathrm{min}(\omega_j)=1$ we set $df$ for AL to
the number of nonzero estimated parameters.   As an `expensive' gold standard,
we include an exact solver for MCP penalized regression. As
described in Section \ref{sec:weighted}, {\tt sparsenet} applies 5-fold CV to
...
Table \ref{tab:sumtables} represents only a small portion of our simulation
...
\{100,200\}$ as slow decay.  Our supplemental material contains an additional
128 tables, detailing results for hundreds of data generating processes and
model configurations.  These tables include results on estimation error
(see also Table \ref{tab:esttables} below), on the fitted number of nonzero
parameters, and on sensitivity and false discovery rates with respect to
oracle support.


\vskip .1cm \noindent {\bf Remarks}

\vskip .1cm \noindent $\bullet$~~ AICc selection for one of the three GL $\gamma$-specifications is always able to provide performance near to that of the computationally intensive routine of CV-selected MCP (over a grid in penalty size and scale).  The best GL OOS $R^2$ is never more than 4\% worse than MCP overall and never more than 2\% worse for all but the ${\kappa}=200$ (essentially dense regression) specifications; GL also occasionally outperforms MCP. 
...
\vskip .1cm \noindent $\bullet$~~  For GL1, AICc gives roughly similar performance to 5-fold CV.  For the lasso (GL0), CV outperforms AICc in a dense (big {$\kappa$}) and high signal model, while AICc outperforms CV in low signal settings.  The opposite holds for GL10.   In any case, the supplement shows that AICc is a massive improvement over either BIC or  AIC;  the
...
true coefficients from our {\it sparse} simulation model. 
 Results are averages over 1000 samples from each of the
 possible data generating process configurations, 
including both binary and continuous designs, 
$\rho$ in \{0, 0.5, .9\} and all of our decay values.
The Oracle is MLE fit  
on the true sparse support and the best results are bolded.}
\end{table}
...
effective sparsity; at {$\kappa$} of 10 or even 50) while at slower decay rates
...
and earlier than it does for GL1; only in the ${\kappa}=10$ setting does there
