...
getit <- function(f, rho, s2n, decay){
	...
	fname <- sprintf("results/sim-rho%g-s2n%g-prob0.5-decay%g-%s.txt",
			rho, s2n, decay, f)
	#print(fname)
...
printline <- function(f, rho, s2n, decay){
	...
	minwtb <- min(wtb)
	wtb <- paste(wtb)
	wtb[which(wtb==minwtb)] <- sprintf("{\\bf %g}",minwtb)
	cat(wtb, sep = " & ") 
...
for(s2n in c(2,1,0.5)){
	...
	for(decay in c(10,50,100)){
		for(rho in c(0,0.5,.9))
			printline("r2",rho,s2n,decay)
		if(decay!=100) cat("\\rule{0pt}{3ex}\n")
	}
}