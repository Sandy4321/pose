...
$\bs{\beta}$ (e.g., the MLE restriced to large coefficients).  Suppose each
response can be written $y = f(\bm{x}) + \varepsilon$, where $f$ is the `true'
best predictor for $y \mid\bm{x}$, and $\bm{f} = [f(\bm{x}_1)\cdots
f(\bm{x}_n)]'$.  Say $S \subset 1\ldots p$ with $|S|=s$, denote vectors
restricted to $S$ as $\bs{\beta}_S = [\beta_j:j\in S]'$, etc, and write
$f_S(\bm{x}) = \alpha + \bm{x}'\bs{\beta} =  \alpha + \bm{x}_s'\bs{\beta}_S$
as a sparse approximation  with $\beta_j = 0$ for $j\in S^c$.  It is
...
\begin{prop} \label{sparseapprox}  Consider the weighted $L_1$ minimization from (\ref{l1pen}) in
...
\subsubsection*{Proposition \ref{sparseapprox}} 
...
\end{align*}
since $0 \leq 2(\frac{a}{2}-c)^2 + (b-c)^2 = \frac{a^2}{2}+b^2+3c^2-2c(a+b)$ for any $a,b,c$.  \hfill$\square$
