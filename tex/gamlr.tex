...
\vskip .25cm
\noindent $\bullet$~~ Condition $(ii)$ shows that sign recovery relative to $L_0$ optimal $\bs{\beta}^S$ relies upon the strength of  {\it marginal} correlations with the response.  It directly suggests use of the marginal regression coefficient estimates as weights for an adaptive lasso, as proposed in \cite{huang_adaptive_2008} for the $p>n$ setting.  Indeed, since marginal correlations tend to have a lower sampling variance than conditional coefficients, this strategy should also yield low variance weights and stable estimation.  Our simulation study investigates marginal-correlation-based adaptive lasso.
\vskip .25cm
