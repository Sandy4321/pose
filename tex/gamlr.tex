...
Another central view of this work holds that the GL algorithm and related
...
essential that these paths are evaluated as a whole, and that are good platforms for model selection. Section
\ref{stability} is a study of path stability, focusing on its implications for
...
The key behind efficient path estimation -- having $\bs{\hat\beta}^{t-1}$  close to $\bs{\hat\beta}^t$ -- also motivates our gamma lasso.  One way to decrease the penalty on large signals (for diminishing bias) is to use weighted $L_1$ costs $c_j(\beta_j) = \omega_j |\beta_j|$ with $\omega_j>0$ small for large $|\beta_j|$.  In particular,  we specify the penalty weights on $\bs{\hat\beta}^t$ at segment $t$, $\bs{\omega}^t$, as decreasing functions of the previous estimates $\bs{\hat\beta}^{t-1}$.
...
Behavior of the GL paths  is governed by
$\gamma$, which we refer to as the penalty scale (see Section
\ref{bayes}).  Under $\gamma=0$ Algorithm \ref{gammalasso} is just the usual lasso. Bias diminishes faster for larger $\gamma$ and, at the  extreme,
...
\ref{gamlr_eg} shows solutions in a simple
problem. 
...
This unbiasedness for large signals is the `diminishing bias' property
...
it is a necessary condition for a penalized estimator to have oracle
...
results specific to log penalization. Oracle properties tend to make strong
assumptions about near-sparsity of true $\bs{\beta}$, but they form a popular
approximation framework for evaluating   high-dimensional estimators and have
motivated an explosion of interest in concave penalties over the past decade.
...
estimation, with a focus on implementations of these ideas and their relationship to weighted $L_1$ penalization.
...
The log cost function is a simple concave penalty.  Another  straightforward
option is the bridge of \cite{frank_statistical_1993}, where $c_j(\beta_j) =
|\bs{\beta}_1|^{q}$ for $q \in (0,1)$.  However, both of these penalties can
lead to discontinuous solution paths:  e.g., consider how the minimizer of the solid line 
of the right panel in Figure \ref{solution} will jump from 0 to
3.5 depending upon small jitter to $B$. \cite{fan_variable_2001} devised their
SCAD penalty as an alternative concave cost function that avoids such solution
discontinuity while retaining the desirable diminishing bias of log and bridge
penalization.  The MCP penalty of \citet{zhang_nearly_2010} is another popular
concave cost function that avoids discontinuity. Both SCAD and MCP are {\it
folded concave penalties}
\citep{fan_strong_2014}:
piecewise functions specifically constructed to satisfy oracle
 requirements.

Many solvers have been contributed to accompany the literature on concave
regularization. SCAD and MCP penalties are implemented  in the {\tt ncvreg}
{\sf R} package \citep{breheny_coordinate_2011}, and this appears to be a
commonly used software for these methods. Unfortunately, the solvers require
...
minutes, limiting the technology to small data analysis problems. This slowness is due to the difficulty of concave minimization:
despite the solution-continuity property SCAD and MCP, their penalized
objective surfaces can be concave and contain multiple minor modes. Finding
global, or even good local, minima in concave functions is an extremely tough
task.  Similarly,
\cite{taddy_multinomial_2013} estimates log penalized objectives through a series
of quadratic approximations, but the method prohibitively expensive for $p$ and $n$
above a few thousand.

One approach that manages to quickly find good solutions under concave
penalization is the SparseNet framework of \citet{mazumder_sparsenet_2011}, a
fast class of coordinate descent routines for generic concave penalized path
estimation. Their algorithm uses coordinate descent to first fit a lasso path
and, for each $\lambda$ on this path, adapts coefficient estimates along a
second grid of increasing concavity specifications (analogous to increasing
values of our $\gamma$). The authors show that moving through a grid of both
$\lambda$ and $\gamma$ in this way helps to avoid minor modes and leads to
faster convergence. An implementation of the MCP penalty is in the {\tt
sparsenet} package for {\sf R}, and it is the best  software we've
found for folded concave penalization. Indeed,  runtimes for {\tt sparsenet} and
roughly equivalent loops of {\tt gamlr} over multiple $\gamma$ are similar.
The (significant) advantage of {\tt gamlr} is that each different $\gamma$
specification can be run in parallel, while {\tt sparsenet} is inherently serial over
both $\gamma$ and $\lambda$ sequences.
...
They also argue that this tends to obtain lower local
minima than will be found using common linear approximation tools
, although Section \ref{lla} details how
striving for the best possible minimum is often not worth the effort.  

 Weighted $L_1$ minimization only ever attempts to solve convex
optimization problems along the path, so convergence to minor modes and slow-to-solve concave objectives are non-issues.  In any case, it seems reasonable to
not worry too much about global solutions under a fixed penalty when
the optimal specification of that penalty is itself unknown:  properties of the
entire path (e.g. stability) are more important than a single point being
optimal under a fixed specification. 
