...
~~~~~~~~~~\mr{(ii)}~~~\frac{1}{n}\bs{x}_j'\bm{y} > \lambda\omega_j ~~\forall~j ~\in~S.
\end{equation}
\end{theorem}
...
1 - \left|\frac{x_j'\bm{e}^S}{n}\right| \geq 1 - \frac{\sqrt{2\nu}}{\lambda\omega_j} \geq |\bs{x}_j'\bm{X}_S(\bm{X}_S'\bm{X}_S)^{-1}\bs{\omega}_S|.
\end{equation}
Finally, for sign recovery on $j\in S$ we need (with inequalities implied on all elements of the compared vectors)
\begin{equation}
|\bs{\beta}_S^\nu| > |\bs{\hat\beta}_S - \bs{\beta}^\nu_S| = n\lambda (\bm{X}_S'\bm{X}_S)^{-1}|\bs{\zeta}_S|.
\end{equation}
This holds if $\bm{X}_S'\bm{X}_S|\bs{\beta}^\nu_S| = |\bm{X}_s'\bm{y}| > n\lambda\bs{\omega}_S$.
...
\vskip .25cm
\noindent $\bullet$~~ A path-wise interpretation of Theorem \ref{signrecov} is that the gamma lasso achieves exact sign recovery if $(i)$ holds until $(ii)$ occurs.  We can re-state the result in terms of false positives relative to $\bs{\beta}^{\nu}$ along the path, with $\bs{\hat\beta}^t$ and $\hat S_t = \{j: \hat\beta^t_j\neq 0\}$ the  weighted lasso coefficient estimates and support under $\bs{\omega}^t$ and $\lambda_t$ at path segment $t$.
\begin{corollary}
...
\vskip .25cm
\noindent $\bullet$~~ Condition $(ii)$ shows that sign recovery relative to the $L_0$-optimal $\bs{\beta}^\nu$ relies upon the strength of  {\it marginal} correlations with the response.  It directly suggests use of the marginal regression coefficient estimates as weights for an adaptive lasso, as proposed in \cite{huang_adaptive_2008} for the $p>n$ setting.  Indeed, since marginal correlations tend to have a lower sampling variance than conditional coefficients, this strategy should also yield low variance weights and stable estimation.  Our simulation study investigates marginal-correlation-based adaptive lasso.
\vskip .25cm
