...
recipe is used to review the literature on diminishing-bias penalty
regularization, emphasizing connections to weighted-$L_1$ penalties, the role
of regularization paths and model selection, and the distance from a weighted
lasso solution to an $L_0$ penalized oracle. The work is illustrated in
...
Section \ref{concave},  recent literature is recommends the use of approximate
...
estimation (OSE) is thus finding starts that are, indeed, good enough.
...
Section \ref{rp} acts as a short survey of sparse diminishing-bias
regularization: we describe regularization paths in general in \ref{rp}.1,
review the connection between concave and weighted-$L_1$ penalties in
\ref{rp}.2, and presents novel results on the distance between weighted-$L_1$
 and $L_0$ minimization in \ref{rp}.3. The POSE algorithm is introduced in
\ref{pose}, with our specific gamma lasso version of the algorithm derived
from Bayesian foundations in \ref{pose}.1.  Section \ref{sss} then covers,
from the perspective of this gamma lasso, the related implementation issues
mentioned above: path speed and stability in \ref{sss}.1, and model selection
tools in \ref{sss}.2.  Finally, Section \ref{eg} presents two empirical studies:
\ref{eg}.1 contains an extensive simulation experiment, and in \ref{eg}.2 we investigate the following data analysis question: given all goals in the past decade of NHL hockey, and information about who was on the ice, what can we say about individual player contributions? 

Throughout, we focus on practice and computation rather than revisiting
statistical properties of the estimators.  For example, the theoretical
results in Section \ref{rp}.3 and the simulation study in Section \ref{eg}.1 take
$L_0$ minimization as our oracle comparator. We remain agnostic about
existence of any sparse `truth' and promote the applicability of these
techniques from a pure information-compression perspective. And Section \ref{sss} emphasizes the ability to quickly obtain and select amongst a range of
penalty specifications, as required by any practitioner who is (as we are)
unwilling to rely upon theoretical derivation for penalty size when analyzing
real data. The goal is to help push sparse diminishing-bias
regularization into the Big Data mainstream.\footnote{To this end, we also
provide polished and well-documented software in the
...
model selection.  In Section \ref{sss} and afterwards, we see that path
...
For example,  one run of SCAD via
...
seconds for the lasso.  This slowness is due to the difficulty of concave
...
path of increasing penalty concavity.  While compute times for
\texttt{sparsenet} are still much higher than for lasso (e.g., 15-20 seconds
in our simulation), it is solving over a large set of penalty
specifications during this time. Indeed, our gamma lasso takes from
\texttt{sparsenet} the idea of moving along a path of closely related
specifications, but does not require grids in both  cost size and concavity.
Intuitively, a single GL run takes a path diagonally through this
concavity-size grid.

Given the computational cost of exact solvers, , 
...
Section \ref{eg}.1).  Like GL, AL provides
...
\ref{eg}.1 provides a simulation study.
...
essential that these paths are evaluated as a whole, and that they are good platforms for model selection.
...
estimator in Section \ref{sss}.2.  Many popular concave cost functions, such as
...
\ref{sss}, the SURE framework of \cite{stein_estimation_1981} applies
...
in Section \ref{eg}.2), we apply (\ref{edf}) directly despite its
...
Since the AICc plays a central role in our applied work, we present below an informal derivation based on the Mallows $C_p$ formula.  The goal here is to make the result as intuitive as possible.  See \cite{claeskens_model_2008} for a more traditional derivation and full theoretical review of information criteria, and refer to \citet{flynn_efficiency_2013} for results extending to generalized linear models (which we take for granted in the hockey application of Section \ref{eg}.2). 
