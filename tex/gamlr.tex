...
the associated response as $\bm{y} = [y_1,\ldots,y_n]'$. Since the size of
penalized $\beta_j$ depends upon the units of $x_{ij}$,  it is common to scale
...
One advantage of diminishing bias is that you reduce the difference between
estimated $\hat y$ and prediction from an optimal sparse approximation to
$\bs{\beta}$ (e.g., the MLE restriced to large coefficients).  Say $S \subset 1\ldots p$ with $|S|=s$, denote vectors
restricted to $S$ as $\bs{\beta}_S = [\beta_j:j\in S]'$, $\bm{X}_S$, etc, and write
$f_S(\bm{x}) = \alpha + \bm{x}'\bs{\beta} =  \alpha + \bm{x}_s'\bs{\beta}_S$
as a sparse approximation  with $\beta_j = 0$ for $j\in S^c$, and say 
$\bm{f}_S = [f_S(\bm{x}_1)\cdots
f_S(\bm{x}_n)]'$. 

\begin{definition}
$\phi^2(L,S)$ is the restricted eigenvalue
$
\phi^2(L,S) = \min_{\{\bm{v}: |\bm{v}_{S^c}| \leq L\|\bm{v}_S\|\}}\bm{v}'\bs{\Sigma}\bm{v}
$ defined on gram matrix $\bs{\Sigma} = \bm{X}'\bm{X}/n$.
\end{definition}

\begin{prop} \label{sparseapprox}  Consider squared-error loss $l(\bs{\beta}) =
\frac{1}{2}\|\bm{X}\bs{\beta}-\bm{y}\|^2$, and suppose $\bs{\beta}_S$ optimizes the $L_0$ penalized objective $l(\bs{\beta}) + n\nu$ with $S<n$.  Write $\bs{\hat\beta}$ as solution to the weighted $L_1$ minimization from $l(\bs{\beta}) + n\lambda\bs{\omega}'|\bs{\beta}|$.

Then  
$\omega^{\mr{min}}_{S^c}\lambda > \sqrt{2\nu}$ while $\phi^2(L,S) > 0$ for 
 $L = \|\bs{\omega}_S\|/(\omega^{\mr{min}}_{S^c}-\sqrt{2\nu}/\lambda)$ implies
\begin{equation} \frac{\|\bm{X}(\bs{\hat\beta}-\bs{\beta}_S)\|^2}{n}\leq
\frac{4\lambda^2 \|\bs{\omega}_S\|^2}{\phi^2(L, S)}
\end{equation} 
\end{prop}
\begin{proof}
Say $\bm{e} = \bm{y}-\bm{f}_s = (\bm{I}-\bm{H}_S)\bm{y}$ where 
$\bm{H}_S = \bm{X}_S(\bm{X}_S'\bm{X}_S)^{-1}\bm{X}_S'$, so that
\begin{equation}
\|\bm{\hat y}-\bm{y}\|^2 = \|\bm{\hat y}-\bm{f}_S + \bm{e}\|^2
= \|\bm{\hat y}-\bm{f}_S\|^2 + \|\bm{e}\|^2 - 2\bm{\hat y}'\bm{e}.
\end{equation}
Also, 
\begin{equation}\bm{\hat y}'\bm{e} = \bm{\hat y}'(\bm{I}-\bm{H}_S)\bm{y} =
\bs{\hat\beta}'\bm{X}'(\bm{y}-\bm{X}\bs{\beta}_S) = 
\sum_{j\in S^c} \hat\beta_j\bs{x}_j'(\bm{y}-\bm{X}\bs{\beta}_S)
\end{equation}
and 
\begin{equation}
\left(\frac{\bs{x}_j'(\bm{y}-\bm{X}\bs{\beta}_S)}{n}\right)^2
\leq \mr{MSE}_S - \mr{MSE}_{S+j} < 2\nu 
\end{equation}
So that $|\bs{x}_j'(\bm{y}-\bm{X}\bs{\beta}_S)| < n\sqrt{2\nu}~~\forall j$.

Hence,
\begin{align}
\frac{\|\bm{X}(\bs{\hat\beta}-\bs{\beta}_S)\|^2}{2} 
  + n\lambda\bs{\omega}_{S^c}'|\bs{\hat\beta}_{S^c}| - \bm{\hat y}'(\bm{I}-\bm{H}_S)\bm{y}
  &~\leq~ n\lambda\bs{\omega}_{S}'|\bs{\hat\beta}_{S}-\bs{\beta}_S| \\
  \Rightarrow ~~ \frac{\|\bm{X}(\bs{\hat\beta}-\bs{\beta})\|^2}{2}
  + n\left(\lambda\omega^{\mr{min}}_{S^c}-\sqrt{2\nu}\right)|\bs{\hat\beta}_{S^c}|
  &~\leq~ n\lambda\|\bs{\omega}_S\|\|\bs{\hat\beta}_S-\bs{\beta}_S\|\\
    \Rightarrow ~~ \frac{\|\bm{X}(\bs{\hat\beta}-\bs{\beta})\|^2}{2}
  &~\leq~ \frac{n\lambda\|\bs{\omega}_S\|\|\bm{X}(\bs{\hat\beta}-\bs{\beta})\|}
  {\sqrt{n}\phi(L, S)}\\
 \Rightarrow ~~ \frac{\|\bm{X}(\bs{\hat\beta}-\bs{\beta})\|}{\sqrt{n}}
 &~\leq~\frac{2\lambda \|\bs{\omega}_S\|}{\phi(L, S)}
\end{align}
...
$\bullet$  We refer to {\it stagewise} regression as iterative fitting of new covariates to the residuals of an existing linear model; the term has more recently been appropriated by HTF as a version of LARS. 

$\bullet$ $\bs{x}_j$ the $j^{th}$ design column.
