...
Log cost is a simple concave penalty.   However, it can lead to discontinuous
solution paths:  e.g., the minimizer of the solid line of the right panel in
Figure \ref{solution} will jump from 0 to 3.5 depending upon small jitter
either to data $B$ or cost $\gamma s$. In contrast, the SCAD penalty of
\cite{fan_variable_2001} and the MCP of \citet{zhang_nearly_2010}, popular
types of {\it folded concave penalties}
\citep[e.g.,][]{fan_strong_2014}, 
which avoid solution discontinuity while retaining
diminishing bias. 
...
CD of Appendix \ref{implement}), and one iterates between updating
...
(e.g., guarantees that an oracle estimator is on the path) and construct
...
-- GL, LLA OSE, adaptive lassos -- differ only in how the weights are
...
superscripted $\bs{\beta}^S$ to denote the coefficient vector for OLS
...
\end{equation}
\end{definition}
\noindent (\ref{irrep}) is often presented with $\bm{v}=\bm{1}$.\footnote{\cite{wainwright_sharp_2009} shows that the irrepresentable condition with $\theta=1$ and $\bm{v}=\bm{1}$ is necessary for lasso sign recovery in the {\it noiseless} estimation setting.} It
...
The result follows directly from the sign recovery lemma in Appendix
\ref{extraproof}. 
\begin{theorem}
...
The benefits of diminishing bias come at a price: if the penalty is concave,
the negative log posterior can be concave.  This can be seen
in Figure \ref{solution}: the objectives
are concave approaching the origin, and can even become multi-modal.
It is  impractical to find the global minimum of such concave functions.
But even if one seeks only a local solution,  concavity can lead to problems
in estimation. As outlined in Section
\ref{concavecosts}, multiple modes in the objective can cause local
solvers to {\it jump} from one mode to another when the problem changes
slightly.

The consequence of these discontinuities is that model fit moves dramatically
in response to small change in {\it either} data $\bm{y}$ or penalty size
$\lambda$.  From the first perspective, estimators that make large changes under
small data jitter are {\it instable}.  This hurts predictive performance by
increasing predictor variance. Moreover, as covered in
\cite{breiman_heuristics_1996}, the variability of
choices made in model selection (see Section \ref{selection}) explodes with
the estimation variance for each candidate model. In Section \ref{dof} we see
also that model complexity -- a key input to selection -- can only be
estimated reliably for stable models. From the second perspective, a fit that
is very {\it sensitive} to specification is unreliable when there is
any model uncertainty.  More immediately, if  $\bs{\hat\beta}$
estimates jump along the regularization path -- that is, if the paths are
non-smooth functions of $\lambda$ -- then $\bs{\hat\beta}^{t-1}$ will not be a
good hot-start for $\bs{\hat\beta}^t$ and  computation time will
quickly grow.
...
$\mr{var}(\tau_j) < n/4$ to get the same guarantee. 
%And for marginal rather than joint MAP estimation you need $\mr{var}(\tau_j) + \gamma^2$ less than $n$ or $n/4$.
...
\end{proof}
\noindent Note that the subset selection procedure implied at $\gamma=\infty$ is clearly
{\it discontinous}.

\subsection{Practical Stability}

In practice, a regularization path can be Lipschitz continuous but still overly sensitive to data jitter and penalty size.  The most obvious effect is computational:  when solutions change too quickly along the path, you lose the benefit of hot-starts and computation time jumps for each segment. As an illustration, Figure \ref{nhltime} shows timings growing rapidly with large $\gamma$ for the hockey data  of Section \ref{nhl}, even though all of these specifications are theoretically stable.
...
model, $df$ is just the number of coefficients.  More generally
\citep[e.g.][]{efron_least_2004}, for $\bm{y} \sim (\bs{\mu},\sigma^2\bm{I})$
we define $df = \sigma^{-2} \sum_i \mr{cov}(\hat y_i, y_i)$ where $\hat y_i$
is the estimated prediction rule for observation $y_i$. Given an estimator
...
GL algorithm each $\tau_j$ is a function of $y$.  Moreover, estimated
$\hat\tau_j$ have been optimized to fit the data and   plugging them into
(\ref{sure}) yields a biased (low) estimate of $df$. Instead, we take
advantage of the Bayesian  hierarchical model of Section \ref{bayes} and
average (\ref{sure}) over the $\mr{Ga}(s,1/\gamma)$ prior on $\tau_j$.
This implies the {\it gamma lasso estimator for degrees of freedom}
...
1/\gamma), \end{equation} where here $\mr{Ga}$ denotes the Gamma distribution
function. As $\gamma \rightarrow 0$, $df^t \rightarrow \hat
p$, the lasso $df$ of Zou et al, and as $\gamma \rightarrow
\infty$, $df^t$ becomes $p$ for all segments of subset selection.
(
...
\hat p$ for $\gamma = 0$.  And for non-Gaussian models (e.g.,
...
perform as well or better than nonparametric cross-validation techniques over
a wide range of $\gamma$ values.
...
Apart from the simulation study, we have not provided extensive
comparison of the many available weighting mechanisms. Although
...
theoretically optimal. For example, instead of moving {\it forward} from higher to lower penalties, as we do,
...
we have found no other software where standard is met.
