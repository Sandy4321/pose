...
recipe is used to review the literature on concave penalty regularization,
...
solution to optimal $L_0$ penalized estimation. The work is illustrated in
...
regularization, where penalties are spiked at zero (so the solution can be
sparse) but have costs diminish so as to remove bias from estimation of large
signals.
...
business and economic collaborations.   Thus Section \ref{theory} presents a
set of novel results on the distance between weighted $L_1$ estimation and an $L_0$ penalized oracle solution (rather than  some assumed
sparse truth), and the simulation study of Section \ref{sim} provides an
empirical investigation of this comparison.
...
essential that these paths are evaluated as a whole, and that they have
properties to make them good platforms for model selection. Section
\ref{stability} is a study of path stability, focusing on its implications for
algorithm speed (so that entire paths are easy to obtain) and for predictor
variance under jitter in $\bm{y}$ (so that we have good tools for model
selection).  Section \ref{dof} describes a heuristic for model
degrees of freedom, and Section \ref{selection} details cross validation and
information criteria as tools available for model selection whenever the paths are stable.
...
focus on the true input signal. Large $\lambda$ lead to very simple and sparse
model estimates, while as $\lambda \rightarrow 0$ we approach maximum
likelihood estimation (MLE). Since you don't know optimal $\lambda$ a priori,
...
\ref{models}), and proceed as $\lambda^t = \delta \lambda^{t-1}$ for $\delta
\in (0,1)$ down to some pre-specified $\lambda^T$ (e.g., $\lambda^T=
0.01\lambda^1$).
...
The key behind efficient path estimation -- that $\bs{\beta}^{t-1}$ is close to $\bs{\beta}^t$ -- also motivates our gamma lasso.  One way to decrease the penalty on large signals (for diminishing bias) is to use weighted $L_1$ costs $c_j(\beta_j) = \omega_j |\beta_j|$ with $\omega_j$ small for large $|\beta_j|$.  In particular,  we specify the penalty weights on $\bs{\hat\beta}^t$ at segment $t$, $\bs{\omega}^t$, as decreasing functions of the previous estimates $\bs{\hat\beta}^{t-1}$.
...
problem. Moving from the lasso ($\gamma=0$) to faster diminishing bias (larger
$\gamma$), we get see that $\bs{\hat\beta}$ moves more quickly as a function of $\lambda$.
...
\subsection{Gradient, curvature, and path starts}
\label{models}
